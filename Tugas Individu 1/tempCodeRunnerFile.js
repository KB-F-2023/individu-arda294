...
const aStar = (adj, start, dest) => {
    // Parent, Current, A* Score
    let visited = [[start, start, getDistance(adj[start], adj[dest])]]
    pq = PriorityQueue((a, b) => a[2] - b[2])
    // Parent, Current, Cost, A* Score
    pq.push([start, start, 0, getDistance(adj[start], adj[dest])])
    let path = []
    while(pq.size) {
        // Returns path
